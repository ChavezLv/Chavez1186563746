...
int sendfile(pWork_t pWork){
    ...
    sprintf(path, "/home/charlie/Netdisk/Net_work_4.0/server/files/%s", md5);
    puts(path);
...
i   printf("remainLen: %ld\n",remainLen);
    ...
    if(file_stat.st_size >= (104857600 * 10)){
    sendLen = 4096;
    send(clientFd, &sendLen, sizeof(sendLen), 0);
    int fds[2];
    pipe(fds);
...
int recvfile(pWork_t pWork){
    ...
    sprintf(path, "/home/charlie/Netdisk/Net_work_4.0/server/files/%s", fileMd5);
    int ret = 0;
    DataTrain_t train;
    memset(&train, 0, sizeof(train));

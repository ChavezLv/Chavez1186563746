...
int main()
{
    while(1)//防止进程意外退出
    {
        ...
        if(fork())
        {
            ...
            printf("workingdir changed.\n");
            //写日志
            MYSQL *conn;
            CONNECT_MYSQL;
            char query[1000]={0};
            sprintf(query,"insert into log_list (username,log) values ('SERVER','***SERVER START***');");
            printf("Log:insert into log_list (username,log) values ('SERVER','***SERVER START***');\n");
            ...
            while(1)
            {
                ...
                for(i=0;i<readyFdCount;i++)
                {
                    if(pEvents[i].data.fd==socketFd)
                    {
                        ...
                        sprintf(query,"insert into log_list (username,log) values ('SERVER','get NewFd=%d');",newFd);
                        printf("insert into log_list (username,log) values ('SERVER','get NewFd=%d');\n",newFd);
                        if(mysql_query(conn,query)){
                            printf("Log ERROR connect mysql:%s\n",mysql_error(conn));
                            return -1;
                        }
                    ...
                    if(pEvents[i].data.fd==fds_exit[0])//收到退出信号
                    {
                        ...
                        CONNECT_MYSQL;
                        bzero(query,sizeof(query));
                        sprintf(query,"insert into log_list (username,log) values ('SERVER','***SERVER END***');");
                        printf("Log:insert into log_list (username,log) values ('SERVER','***SERVER END***');\n");
                        if(mysql_query(conn,query)){
                            printf("Log ERROR connect mysql:%s\n",mysql_error(conn));
                            return -1;
                        }

...
{//v4: 
	{
	MutexLockGuard autolock(_mutex);//临界资源vector不是线程安全的
	...
	if(0 == nready) {
		...
		for(int idx = 0; idx != nready; ++idx) {
			...
			if(fd == _acceptor.fd()) {
				...
				if(_evtList[idx].events & EPOLLIN) {//最好是判断一下?
					handleRead();
					doPendingFunctors();
...
{//
	...
	/*for(auto& func: _pendingFunctors){
		functor();//这方式不太好, 因为操作过程vector还在变的, 影响体验
	}*/
	vector<Functor> tmp;
	{
	MutexLockGuard autolock(_mutex);
	_pendingFunctors.swap(tmp); // O(1),vector底层三个指针
	}

...
void test(){
    ...
    int number = 100;
    const char *pstr = "Signal";
    logError("error %d occured at %d line! %s", number, __LINE__, pstr);
    logInfo("get info %d at %d line! %s", 10, __LINE__, pstr);
    logWarn("warn %s at line %d!", pstr, __LINE__);
    /* logError("msg"); */
    /* logError("msg"); */
    /* logError("msg"); */
}

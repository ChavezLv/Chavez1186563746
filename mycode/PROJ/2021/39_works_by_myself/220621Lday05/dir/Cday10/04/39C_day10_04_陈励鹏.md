...
void headle(char* str);
void insert(char* hashTable, char* str,int n);
...
int main()
{
	...
	int hashTable[MAXKEY] = { 0 }; //hashTable存的是索引
	...
	for (int i = 0; i < 5; i++)
	{
		insert(hashTable, save_ptr[i],i+1);
		int index = hash(str);
...
void insert(int* hashTable, char str[],int n)
{
	char b[30] = {0};
	int  j = 0, flag = 0; //flag=1判断前面的字符串加好了没
	for (int i = 0; str[i]; i++) //0和' '是不一样的
	{
		if (str[i] != ' ') {
			b[j++] = str[i];
			b[j] = 0;

...
/*
(a). 假设数组是先单调递增到一个峰值，然后单调递减的。
设计并实现算法找到峰值元素的下标。
*/

int findMaxPeak(int *arr) {

	int i, j;
	for (i = 0, j = 1; arr[j] - arr[i] >= 0; ++i, ++j) {
...
int main() {
	...
	int arr[N] = {1,2,3,4,5,6,5,4,3,1};
	printf("%d ",findMaxPeak(arr));
	return 0;
}
```
...
/*
(b). 下面是一篇文章的段落的若干个句子。
每个句子都存在一个编号。从键盘中输入一个单词，
判断这个单词有没有在段落中出现过，如果出现过，
输出其第一次出现时所在句子的编号。
*/


int findFirstTarget(char *sentence,char *word) {
	int i, j;
	for (i = 0, j = 0; sentence[i];) {
		//不区分大小写
		if (sentence[i] == word[j] || sentence[i] + 32 == word[j] || sentence[i] - 32 == word[j]) {
			++i;
			++j;
			if (word[j] == '\0') {
				return 1;
...
int main() {
	...
	for (int i = 0; i < 5; ++i) {
		if (findFirstTarget(sentence + i, word)) {
			printf("%d\n", i + 1);
			return 0;
		}
	}

...
```c
void merge(int *arr, int left, int mid, int right, int *brr) {//归并
	int i, j, k;
...
void mergeSort(int *arr, int left, int right, int *brr) {//计数排序
	if (left < right) {
...
```c
针对42亿的整型数int的变化范围，用位图处理所需要的空间是512M，因为是（4G/8,每个字节是8位）
```

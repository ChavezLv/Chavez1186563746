...
void sigFunc(int sigNum){
    PRINT_CHECK(sigNum);
    write(exitPipe[1], &sigNum, sizeof(sigNum));
}
    ...
    while(1){
        ...
        for(int i = 0; i < readyNum; ++i){ //客户端连接服务器，使用accept函数接收这次连接请求，返回newFd， 交给空闲子进程
            if(events[i].data.fd == sockFd){
                ...
                for(int j = 0; j < processPoolNum; ++j){
                    if(0 == pProcessPool[j].flag){
                        sendFd(pProcessPool[j].pipeFd, newFd, 0);
                        /* printf("j = %d\n",j); */
                        /* pProcessPool->flag = 1; */
            ...
            if(events[i].data.fd == exitPipe[0]){
                /* ERROR_CHECK(readyNum, -1, "epoll_wait"); */
                for(int j = 0; j < processPoolNum; ++j){
                    sendFd(pProcessPool[j].pipeFd, 0, 1);
                    /* printf("main.c sendFd success\n"); */
                }
                for(int j = 0; j < processPoolNum; ++j){
                    //wait一次回收一个子进程
                    wait(NULL);  // or  wait(0);

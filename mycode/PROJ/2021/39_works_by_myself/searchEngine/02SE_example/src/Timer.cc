...
Timer::Timer(TimerCallback &&cb,int initialTime,int periodicTime)
    :_tfd(createTimerfd()),_cb(std::move(cb)),_initialTime(initialTime)
     ,_periodicTime(periodicTime),_isStarted(false)
    {}
Timer::~Timer()
{
    if(_isStarted)
...
void Timer::start()
{
    ...
    while(_isStarted){
        ...
        }while(nready==-1&&errno==EINTR);//异常中断
        if(nready==-1)
        {
            perror("poll");
            return;
...
void Timer::handleRead()//处理定时器操作
{
    ...
    if(ret!=sizeof(howmany))//才8字节，读不到肯定出错了
    {
        perror("read");
    }
}

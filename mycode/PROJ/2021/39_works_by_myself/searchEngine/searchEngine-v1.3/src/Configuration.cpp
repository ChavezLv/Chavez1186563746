...
/* } */
unordered_map<int, shared_ptr<WebPage> > & Configuration::getPageLib(){
    if(0 == _pageLib.size()){
        operator<<(cout, "Configuration::page lib file not exist");
    }
    return _pageLib;
...
void Configuration::loadDataFiles(){
    ...
    while(getline(ifsOffset, line)){
        ...
        std::shared_ptr<WebPage> uqWeb(new WebPage(doc, this, _jieba));
/* cout << uqWeb->getTitle() << endl; */ 
        _pageLib.insert(make_pair(docId, uqWeb) );
    ...
    cout << "Configuration:pagelib&offsetlib&invertlib data files ready!" << endl;

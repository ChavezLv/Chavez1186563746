...
cout << " retend WebPageQuery::doQuery(const string &sought)" << endl;
        return jsonLayOutString(docIDVec, queryWords);

    }else{
cout << " nullend WebPageQuery::doQuery(const string &sought)" << endl;
...
/* cout << "WebPageQuery::executeQuery array " << eachPairOfWord->first->first << ":" << (eachPairOfWord + 1)->first->first << endl; */ 

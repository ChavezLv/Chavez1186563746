...
class Sender : public de::Thread
{
 public:
  Sender() {
    pin.SetTypeInfo(0, "de::benchmark::SenderTask", de::TaskDeSerializeCustom<de::benchmark::SenderTask>, de::TaskDeleter<de::benchmark::SenderTask>);
    pouts_[0]->SetTypeInfo("de::benchmark::SenderTask", de::TaskSerializeCustom<de::benchmark::SenderTask>);
    SetName("sender");
  }
  ~Sender() {
    //删除入队列，停止算子
...
//注册全局node
DE_CLASS_REGISTER("de::benchmark::Sender", Sender);
DE_CLASS_REGISTER("de::benchmark::Receiver", Receiver);
...
DE_REGISTER_GLOBAL("de.benchmark.predict.init").
  ...
  par->dev_id = dev_id;
  int interval_type = args[13];
  int interval_time = args[14];
  ...
  //创建node
  graph->CreateHostNode("de::benchmark::Sender", "Sender");
  graph->CreateDevNode("de::benchmark::AiEngineTest", "AiEngine");
  graph->CreateHostNode("de::benchmark::Receiver", "Receiver");
  if (decode_onchip == 1) graph->CreateDevNode("de::JpegDecoder", "JpegDecNode");
  graph->CreateDevNode("de::benchmark::DevReceiver", "DevReceiver");
  graph->CreateDevNode("de::benchmark::DevSender", "DevSender");
  ...
  graph->SetNodeAttr("DevReceiver", "interval_type", (int)interval_type);
  graph->SetNodeAttr("DevReceiver", "interval_time", (int)interval_time);
...
DE_REGISTER_GLOBAL("de.benchmark.predict.setinput").
  ...
  int64_t use_raw_file = args[7]; 
  int64_t raw_file_type = args[8];
  ...
  if(use_raw_file){
    std::vector<int64_t> shapes;
    DEType type = (raw_file_type==2)?String2DEType("float"):String2DEType("uint8");
    shapes.push_back((raw_file_type==2)?shape0/4:shape0);
    shapes.push_back(shape1);
    ndarray = NDArray::Empty(shapes, type, DEContext{ kDLCPU, 0 }, { dePixelFormat::DE_PIX_FMT_NONE, deShapeCode::HW, {0,0,0,0}, 256 });
    memcpy(ndarray.GetTensorData(), const_cast<char*>(byte_array->data), ndarray.GetTensorDataSize());
  }
  else
    ndarray = NDArray::Create(shape0, shape1, format, {0, 0, shape1, shape0}, const_cast<char*>(byte_array->data));

...
void GarbageOverflowData::addGroundTruth(YAML::Node &node, std::string filename) {
...
GarbageOverflowUnittest::GarbageOverflowUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

GarbageOverflowUnittest::~GarbageOverflowUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> \
        GarbageOverflowUnittest::createAlgHandle(int batch) {
...
std::shared_ptr<arctern::RunParameter> GarbageOverflowUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::GarbageOverflowRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> GarbageOverflowUnittest::generateGroundTruth() {
  std::shared_ptr<GarbageOverflowData> ptr(new GarbageOverflowData);
...
void GarbageOverflowUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::GarbageOverflowResult *>(p);
  std::shared_ptr<GarbageOverflowData> data = std::dynamic_pointer_cast<GarbageOverflowData>(gdata);

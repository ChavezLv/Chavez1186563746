...
void AdvertisementDetData::addGroundTruth(YAML::Node &node, std::string filename) {
...
AdvertisementDetUnittest::AdvertisementDetUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

AdvertisementDetUnittest::~AdvertisementDetUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> AdvertisementDetUnittest::createAlgHandle(int batch) {
...
std::shared_ptr<arctern::RunParameter> AdvertisementDetUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::AdvertisementDetRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> AdvertisementDetUnittest::generateGroundTruth() {
  std::shared_ptr<AdvertisementDetData> ptr(new AdvertisementDetData);
...
void AdvertisementDetUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::AdvertisementDetResult *>(p);
  std::shared_ptr<AdvertisementDetData> data = std::dynamic_pointer_cast<AdvertisementDetData>(gdata);

...
void LoessDetectTinyData::addGroundTruth(YAML::Node &node, std::string filename) {
...
LoessDetectTinyUnittest::LoessDetectTinyUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

LoessDetectTinyUnittest::~LoessDetectTinyUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> LoessDetectTinyUnittest::createAlgHandle(int batch) {
...
std::shared_ptr<arctern::RunParameter> LoessDetectTinyUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::LoessDetectTinyRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> LoessDetectTinyUnittest::generateGroundTruth() {
  std::shared_ptr<LoessDetectTinyData> ptr(new LoessDetectTinyData);
...
void LoessDetectTinyUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::LoessDetectTinyResult *>(p);
  std::shared_ptr<LoessDetectTinyData> data = std::dynamic_pointer_cast<LoessDetectTinyData>(gdata);

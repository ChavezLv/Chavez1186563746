...
void RoadDamageData::addGroundTruth(YAML::Node &node, std::string filename) {
...
RoadDamageUnittest::RoadDamageUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

RoadDamageUnittest::~RoadDamageUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> RoadDamageUnittest::createAlgHandle(int batch) {
...
std::shared_ptr<arctern::RunParameter> RoadDamageUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::RoadDamageRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> RoadDamageUnittest::generateGroundTruth() {
  std::shared_ptr<RoadDamageData> ptr(new RoadDamageData);
...
void RoadDamageUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::RoadDamageResult *>(p);
  std::shared_ptr<RoadDamageData> data = std::dynamic_pointer_cast<RoadDamageData>(gdata);

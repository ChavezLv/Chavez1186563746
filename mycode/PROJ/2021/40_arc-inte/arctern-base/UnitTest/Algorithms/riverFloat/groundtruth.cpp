...
void RiverFloatData::addGroundTruth(YAML::Node &node, std::string filename) {
...
RiverFloatUnittest::RiverFloatUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

RiverFloatUnittest::~RiverFloatUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> RiverFloatUnittest::createAlgHandle(int batch) {
...
std::shared_ptr<arctern::RunParameter> RiverFloatUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::RiverFloatRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> RiverFloatUnittest::generateGroundTruth() {
  std::shared_ptr<RiverFloatData> ptr(new RiverFloatData);
...
void RiverFloatUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::RiverFloatResult *>(p);
  std::shared_ptr<RiverFloatData> data = std::dynamic_pointer_cast<RiverFloatData>(gdata);

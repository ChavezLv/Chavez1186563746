...
void ObjectDetData::addGroundTruth(YAML::Node &node, std::string filename) {
...
ObjectDetUnittest::ObjectDetUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

ObjectDetUnittest::~ObjectDetUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> ObjectDetUnittest::createAlgHandle(int batch) {
  std::shared_ptr<arctern::ObjectDet> ptr(new arctern::ObjectDet);
...
std::shared_ptr<arctern::RunParameter> ObjectDetUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::ObjectDetRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> ObjectDetUnittest::generateGroundTruth() {
  std::shared_ptr<ObjectDetData> ptr(new ObjectDetData);
...
void ObjectDetUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::ObjectDetResult *>(p);
  std::shared_ptr<ObjectDetData> data = std::dynamic_pointer_cast<ObjectDetData>(gdata);

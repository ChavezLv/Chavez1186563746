...
void IllegalAccumulationData::addGroundTruth(YAML::Node &node, std::string filename) {
...
IllegalAccumulationUnittest::IllegalAccumulationUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

IllegalAccumulationUnittest::~IllegalAccumulationUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> IllegalAccumulationUnittest::createAlgHandle(int batch) {
  std::shared_ptr<arctern::IllegalAccumulation> ptr(new arctern::IllegalAccumulation);
...
std::shared_ptr<arctern::RunParameter> IllegalAccumulationUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::IllegalAccumulationRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> IllegalAccumulationUnittest::generateGroundTruth() {
  std::shared_ptr<IllegalAccumulationData> ptr(new IllegalAccumulationData);
...
void IllegalAccumulationUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::IllegalAccumulationResult *>(p);
  std::shared_ptr<IllegalAccumulationData> data = std::dynamic_pointer_cast<IllegalAccumulationData>(gdata);

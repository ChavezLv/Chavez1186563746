...
void HangStreetDetData::addGroundTruth(YAML::Node &node, std::string filename) {
...
HangStreetDetUnittest::HangStreetDetUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

HangStreetDetUnittest::~HangStreetDetUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> HangStreetDetUnittest::createAlgHandle(int batch) {
...
std::shared_ptr<arctern::RunParameter> HangStreetDetUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::HangStreetDetRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> HangStreetDetUnittest::generateGroundTruth() {
  std::shared_ptr<HangStreetDetData> ptr(new HangStreetDetData);
...
void HangStreetDetUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::HangStreetDetResult *>(p);
  std::shared_ptr<HangStreetDetData> data = std::dynamic_pointer_cast<HangStreetDetData>(gdata);

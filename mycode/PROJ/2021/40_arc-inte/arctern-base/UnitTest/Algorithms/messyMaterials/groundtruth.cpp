...
void MessyMaterialsData::addGroundTruth(YAML::Node &node, std::string filename) {
...
MessyMaterialsUnittest::MessyMaterialsUnittest(std::string ymlPath, std::string imgDir, std::string modelpath,arctern::Result *p)
    : BaseAlgrithmUnittest(ymlPath, "", imgDir, modelpath,p) {
}

MessyMaterialsUnittest::~MessyMaterialsUnittest() {
}

std::shared_ptr<arctern::AlgorithmInterface> MessyMaterialsUnittest::createAlgHandle(int batch) {
...
std::shared_ptr<arctern::RunParameter> MessyMaterialsUnittest::buildRunParameter(std::shared_ptr<BaseGroundTruthData> gdata,
                                                                                 int startId,
                                                                                 int len) {
  std::shared_ptr<arctern::RunParameter> ptr(new arctern::MessyMaterialsRunParameter);
  return ptr;
}

std::shared_ptr<BaseGroundTruthData> MessyMaterialsUnittest::generateGroundTruth() {
  std::shared_ptr<MessyMaterialsData> ptr(new MessyMaterialsData);
...
void MessyMaterialsUnittest::checkResult(std::shared_ptr<BaseGroundTruthData> gdata,
                                         int startId,
                                         int len,
                                         const arctern::Result *p) {
  EXPECT_NE(p, nullptr);

  auto result = dynamic_cast<const arctern::MessyMaterialsResult *>(p);
  std::shared_ptr<MessyMaterialsData> data = std::dynamic_pointer_cast<MessyMaterialsData>(gdata);

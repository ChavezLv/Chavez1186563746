...
#if 0
static std::vector<char> getModel(std::string name) {
  std::ifstream t(name);
  t.seekg(0, std::ios::end);
  int length = t.tellg();
  t.seekg(0, std::ios::beg);
  std::vector<char> result(length);
  t.read(result.data(), length);
  return result;
...
#if 0
  auto raw_model = getModel("face-det-video-0.0.3.rknn");
  std::cout<<"--------------------->"<< raw_model.size()<<std::endl;
#endif

  auto net_params = model->params();

  rknn_context ctx;
  int ret = rknn_init(&ctx, (void *)net_params->data(), net_params->size(), 0, nullptr);
  rknnCtx_.first = ctx;
  if (ret < 0) {
...
void RknnEngin::printTensorAttr(rknn_tensor_attr &attr) {
  std::cout << "index=" << attr.index << " name=" << attr.name
            << " n_dims=" << attr.n_dims << " dims=[" << attr.dims[0] << " "
            << attr.dims[1] << " " << attr.dims[2] << " " << attr.dims[3] << "]"

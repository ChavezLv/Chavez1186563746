    ...
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam)
{
	...
	/*Malloc mem*/
	//"SAMPLE_NNIE_TASK"
	//std::string aa = "SAMPLE_NNIE_TASK";
	//HI_CHAR *pmb =  aa.c_str();
#ifdef DEBUG
  std::cout << "SAMPLE_COMM_SVP_MallocCached bytes: " << u32TotalSize << std::endl;
#endif // ifdef DEBUG
	s32Ret = SAMPLE_COMM_SVP_MallocCached(NULL,NULL,(HI_U64*)&u64PhyAddr,(void**)&pu8VirAddr,u32TotalSize);
  if (s32Ret != HI_SUCCESS) {
    std::cerr << "SAMPLE_COMM_SVP_MallocCached failed: " << std::hex << s32Ret << ", " << u64PhyAddr << ", " << pu8VirAddr << ", " << std::dec << u32TotalSize << std::endl;
  }
	//SAMPLE_SVP_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,SAMPLE_SVP_ERR_LEVEL_ERROR,
    ...
    SAMPLE_SVP_NNIE_MODEL_S *pstNnieModel)
{
	...
	s32Ret = SAMPLE_COMM_SVP_MallocMem((char *) "SAMPLE_NNIE_MODEL", NULL,(HI_U64*)&u64PhyAddr,(void**)&pu8VirAddr,slFileSize);
  if (s32Ret != HI_SUCCESS) {
    std::cerr << "SAMPLE_COMM_SVP_MallocMem failed: " << std::hex << s32Ret << ": " << &u64PhyAddr << ", " << &pu8VirAddr << std::dec << ", " << slFileSize << std::endl;
  }

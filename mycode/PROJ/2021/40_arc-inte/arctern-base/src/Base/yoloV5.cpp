...
int YoloV5::getConfigInfo(arctern::MxnetModel *model) {
  return 0;
}
...
Vbbox YoloV5::postProcess2(float *data0, float *data1, float *data2, int width, int height, float thresh) {
  std::vector<Bbox> output_bboxes;
  parse_yolov5(data0, 80, thresh, output_bboxes);
  parse_yolov5(data1, 40, thresh, output_bboxes);
  parse_yolov5(data2, 20, thresh, output_bboxes);

...
void hungarian_init(hungarian_t *prob, double **r, size_t m, size_t n, int mode) {
  ...
  //
  // TODO(no): automatically transpose such matrices
  assert(m <= n);
  ...
  prob->r = reinterpret_cast<double **>(calloc(m, sizeof(double *)));

  // assert(prob->q = (int**)calloc(m,sizeof(int*)));
  prob->q = reinterpret_cast<int **>(calloc(m, sizeof(int *)));

  prob->mode = mode;
  prob->maxutil = 0;
  for (unsigned int i = 0; i < m; i++) {
    // assert(prob->r[i] = (double*)calloc(n,sizeof(double)));
    prob->r[i] = reinterpret_cast<double *>(calloc(n, sizeof(double)));

    // assert(prob->q[i] = (int*)calloc(n,sizeof(int)));
    prob->q[i] = reinterpret_cast<int *>(calloc(n, sizeof(int)));
  ...
  prob->a = reinterpret_cast<int *>(calloc(m, sizeof(int)));
  prob->u = reinterpret_cast<double *>(calloc(m, sizeof(double)));
  prob->v = reinterpret_cast<double *>(calloc(n, sizeof(double)));
  prob->seq.i = reinterpret_cast<unsigned int *>(calloc(m * n, sizeof(unsigned int)));
  prob->seq.j = reinterpret_cast<unsigned int *>(calloc(m * n, sizeof(unsigned int)));
  prob->ess_rows = reinterpret_cast<int *>(calloc(m, sizeof(int)));
  prob->ess_cols = reinterpret_cast<int *>(calloc(n, sizeof(int)));
}
...
void hungarian_make_cover(hungarian_t *prob) {
  ...
  row_max = reinterpret_cast<double *>(calloc(prob->m, sizeof(double)));
  // assert(col_max = (double*)calloc(prob->n,sizeof(double)));
  col_max = reinterpret_cast<double *>(calloc(prob->n, sizeof(double)));
...
hungarian_print_assignment(hungarian_t *prob) {
  ...
  for (size_t i = 0; i < prob->m; i++) {
    ...
    for (size_t j = 0; j < prob->n; j++)
      printf("%4d ", (static_cast<int>(j) == prob->a[i]) ? 1 : 0);
    printf(" ]\n");
  }
}

...
#ifndef SRC_ALGORITHMS_FACETRACKER_OBJECT_TRACKER_MUNKRES_H_
#define SRC_ALGORITHMS_FACETRACKER_OBJECT_TRACKER_MUNKRES_H_
...
class Munkres {
 ...
 public:
  ...
  static void replace_infinites(HunMatrix<Data> &matrix) {
  ...
  static void minimize_along_direction(HunMatrix<Data> &matrix, const bool over_columns) {
    ...
    for (size_t i = 0; i < outer_size; i++) {
      ...
      for (size_t j = 1; j < inner_size && min > 0; j++) {
        min = std::min(
            min,
            static_cast<double >(over_columns ? matrix(j, i) : matrix(i, j)));
      }
 ...
 private:
...
#ifdef DEBUG
    // std::cout << "Munkres matrix has " << covercount <<
    // " of " << matrix.minsize() << " Columns covered:" << std::endl;
  ...
  HunMatrix<int> mask_matrix;
  HunMatrix<Data> matrix;
  bool *row_mask;
  bool *col_mask;
  size_t saverow = 0, savecol = 0;
};

#endif  // SRC_ALGORITHMS_FACETRACKER_OBJECT_TRACKER_MUNKRES_H_

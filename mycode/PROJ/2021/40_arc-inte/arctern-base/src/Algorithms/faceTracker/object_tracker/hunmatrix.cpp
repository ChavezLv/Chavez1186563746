...
#include "src/Algorithms/faceTracker/object_tracker/hunmatrix.h"
#include <cassert>
#include <cstdlib>
#include <algorithm>

/*export*/ template<class T>
HunMatrix<T>::HunMatrix() {
  m_rows = 0;
  m_columns = 0;
  m_matrix = nullptr;
}
...
/*export*/ template<class T>
HunMatrix<T>::HunMatrix(const std::initializer_list<std::initializer_list<T>> init) {
  m_matrix = nullptr;
  m_rows = init.size();
  if (m_rows == 0) {
    m_columns = 0;
...
/*export*/ template<class T>
HunMatrix<T>::HunMatrix(const HunMatrix<T> &other) {
...
/*export*/ template<class T>
HunMatrix<T>::HunMatrix(const size_t rows, const size_t columns) {
  m_matrix = nullptr;
  resize(rows, columns);
}
...
/*export*/ template<class T>
void
HunMatrix<T>::resize(const size_t rows, const size_t columns, const T default_value) {
...
/*export*/ template<class T>
inline T &
HunMatrix<T>::operator()(const size_t x, const size_t y) {
  assert(x < m_rows);
  assert(y < m_columns);
  assert(m_matrix != nullptr);
  return m_matrix[x][y];
...
/*export*/ template<class T>
inline const T &
HunMatrix<T>::operator()(const size_t x, const size_t y) const {
  assert(x < m_rows);
  assert(y < m_columns);
  assert(m_matrix != nullptr);
  return m_matrix[x][y];
...
template
class HunMatrix<double>;
template
class HunMatrix<float>;
template
class HunMatrix<int>;

...
AtlasRoadDamageImpl::AtlasRoadDamageImpl(){
...
ErrorType AtlasRoadDamageImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasRoadDamageImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  
  auto r = dynamic_cast<RoadDamageResult *>(m_pResult);
  ...
  for (int subId = 0; subId < total; subId++) {
    ...
    for (auto &box : boxes) {
      ROAD_DAMAGE_DETECT_TINY_INFO info;
      info.rect.x = box.x1;
      info.rect.y = box.y1;
...
ErrorType AtlasRoadDamageImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

...
AtlasRiverFloatImpl::AtlasRiverFloatImpl(){
...
ErrorType AtlasRiverFloatImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasRiverFloatImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  
  auto r = dynamic_cast<RiverFloatResult *>(m_pResult);
  ...
  for (int subId = 0; subId < total; subId++) {
    ...
    for (auto &box : boxes) {
      RIVER_FLOAT_DETECT_TINY_INFO info;
      info.rect.x = box.x1;
      info.rect.y = box.y1;
...
ErrorType AtlasRiverFloatImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

...
#include "atlasRubbishDetectTinyImpl.h"
...
AtlasRubbishDetectTinyImpl::AtlasRubbishDetectTinyImpl(){
...
ErrorType AtlasRubbishDetectTinyImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasRubbishDetectTinyImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  ...
  for (int subId = 0; subId < total; subId++) {
    ...
    for (auto &box : boxes) {
      ...
      info.type = (ExposeRubbishType) box.label;
...
ErrorType AtlasRubbishDetectTinyImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

...
#include "atlasIllegalAccumulationImpl.h"
...
AtlasIllegalAccumulationImpl::AtlasIllegalAccumulationImpl(){
...
ErrorType AtlasIllegalAccumulationImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasIllegalAccumulationImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  
  auto r = dynamic_cast<IllegalAccumulationResult *>(m_pResult);
  ...
  for (int subId = 0; subId < total; subId++) {
    ...
    for (auto &box : boxes) {
      ILLEGAL_ACCUMULATION_DETECT_TINY_INFO info;
      info.rect.x = box.x1;
      info.rect.y = box.y1;
      info.rect.width = box.x2 - box.x1;
      info.rect.height = box.y2 - box.y1;
      info.score = box.score;
      info.type = (IllegalAccumulationType) box.label;
...
ErrorType AtlasIllegalAccumulationImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

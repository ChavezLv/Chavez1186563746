...
#include "atlasGarbageOverflowImpl.h"
...
AtlasGarbageOverflowImpl::AtlasGarbageOverflowImpl(){
...
ErrorType AtlasGarbageOverflowImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasGarbageOverflowImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  
  auto r = dynamic_cast<GarbageOverflowResult *>(m_pResult);
  ...
  for (int subId = 0; subId < total; subId++) {
    ...
    for (auto &box : boxes) {
      ...
      info.type = (GarbageOverflowType) box.label;
...
ErrorType AtlasGarbageOverflowImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

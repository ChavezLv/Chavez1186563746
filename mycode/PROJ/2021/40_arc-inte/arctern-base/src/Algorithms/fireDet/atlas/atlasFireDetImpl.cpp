...
AtlasFireDetImpl::AtlasFireDetImpl(){
...
ErrorType AtlasFireDetImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasFireDetImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  
  auto r = dynamic_cast<FireDetResult *>(m_pResult);
...
ErrorType AtlasFireDetImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

...
AtlasObjectDetImpl::AtlasObjectDetImpl(){
...
ErrorType AtlasObjectDetImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasObjectDetImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  
  auto r = dynamic_cast<ObjectDetResult *>(m_pResult);
  ...
  for (int subId = 0; subId < total; subId++) {
    ...
    for (auto &box : boxes) {
      if (box.label == 25 || box.label == 56 || box.label == 60) {
        OBJECT_DETECT_TINY_INFO info;
        info.rect.x = box.x1;
        info.rect.y = box.y1;
        info.rect.width = box.x2 - box.x1;
        info.rect.height = box.y2 - box.y1;
        info.score = box.score;
        info.type = (ObjectType) box.label;
...
ErrorType AtlasObjectDetImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

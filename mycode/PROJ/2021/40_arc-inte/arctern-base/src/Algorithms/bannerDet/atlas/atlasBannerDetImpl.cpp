...
AtlasBannerDetImpl::AtlasBannerDetImpl(){
...
ErrorType AtlasBannerDetImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasBannerDetImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  ...
  for (int subId = 0; subId < total; subId++) {
    ...
    for (auto &box : boxes) {
      ...
      info.type = (BannerType) box.label;
...
ErrorType AtlasBannerDetImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

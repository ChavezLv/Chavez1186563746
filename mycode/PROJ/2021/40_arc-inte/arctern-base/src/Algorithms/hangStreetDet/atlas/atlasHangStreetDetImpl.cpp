...
#include "atlasHangStreetDetImpl.h"
...
AtlasHangStreetDetImpl::AtlasHangStreetDetImpl(){
...
ErrorType AtlasHangStreetDetImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasHangStreetDetImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  
  auto r = dynamic_cast<HangStreetDetResult *>(m_pResult);
...
ErrorType AtlasHangStreetDetImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,

...
ErrorType CenterNet::CheckRunParam(const RunParameter *p) {
...
ErrorType CenterNet::LocalPostProcess(const RunParameter *p,
                                   int startId,
                                   int endId,
                                   std::vector<arctern::Tensor<float>> &outputs) {
  auto r = dynamic_cast<MultiDetectCenterResult *>(result_);
 int total = endId - startId;
  ...
  for (int subId = 0; subId < total; subId++) {
    ...
    if(version_ == "1.0.0"){
      pad_h.push_back(0);
      pad_w.push_back(0);
      img_ratio_w.push_back(imgcols[subId]*1.0/input_w_);
      img_ratio_h.push_back(imgrows[subId]*1.0/input_h_);
      hm_ratio_w_ = 4;
      hm_ratio_h_ = 4;
    }
   else{
  ...
  float* top_inds   = outputs[0].data();

  float* top_scores = outputs[1].data();
  ...
  for(int subId = 0; subId < total; subId++) {
    ...
    for(int j = 0; j < stride_ind; j++){
      ...
      if(top_scores[j + offset_ind] > initParam_.thresh_){
        if (flag_ind_wh) {
...
int CenterNet::GetConfigInfo(arctern::MxnetModel *model) {
  ...
  if(config != nullptr){
    ...
    for(auto it = config->begin();it != config->end(); it++){
      ...
      auto temp = config->LookupByKey(it->key()->str().c_str());
      if(temp!= nullptr){
        std::cout << temp->value()->str() <<std::endl;
      }
      if(it->key()->str() == "INPUT_DIM_W"){
        std::cout <<"finde" << endl;
      }
      configStr+=":";
    }
  ...
  if (initParam_.thresh_ <= 0) {
    return_res &= try_get_value(*model, "CONF_THRESH", initParam_.thresh_);
  }
  ...
  int ret = initMemberVariable();
  return ret;
}

bool CenterNet::initMemberVariable() {
...
void CenterNet::initPostProcessParam(int img_width, int img_height, const cv::Size& osize,
...
void CenterNet::InitPreprocessorParam() {
  ...
  init_pt.alignMent_ = arctern::AlignMent::AlignCenter;
  auto code = preProcessor_->Init(&init_pt);
  assert(code == ErrorType::ERR_SUCCESS);
}

 void CenterNet::InitPreprocessorRunParam(OpencvPreprocessorRunRt *runParm) {
  if(version_ == "1.0.0"){

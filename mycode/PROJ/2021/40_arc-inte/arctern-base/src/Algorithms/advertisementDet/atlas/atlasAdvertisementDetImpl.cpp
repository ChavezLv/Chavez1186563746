...
#include "atlasAdvertisementDetImpl.h"
...
AtlasAdvertisementDetImpl::AtlasAdvertisementDetImpl(){
...
ErrorType AtlasAdvertisementDetImpl::GetModelsConfigInfo(const TrtModels* models){
  assert(models);
...
ErrorType AtlasAdvertisementDetImpl::LocalPostProcess(const RunParameter *p, int startId, int endId,
std::vector<std::vector<float>> &outputs) {
  
  auto r = dynamic_cast<AdvertisementDetResult *>(m_pResult);
...
ErrorType AtlasAdvertisementDetImpl::LocalPreProcess(std::vector<DvppDataInfo>& vInputInfo,
std::vector<DvppDataInfo>& vOutputInfos,
std::vector<cv::Rect>& vRects) {
  
  auto ret = m_pPreprocessor->CombineResizeBatchProcess(vInputInfo, vOutputInfos, true, VpcProcessType::VPC_PT_DEFAULT);

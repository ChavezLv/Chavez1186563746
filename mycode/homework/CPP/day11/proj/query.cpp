...
void TextQuery::readFile(const string filename)
{
    ...
    for(auto itr=strMap.begin();itr!=strMap.end();++itr){
        cout<<itr->first<<":amount="<<itr->second.first\
            <<",line="<<itr->second.second<<endl;
    }
    cout<<endl;
#endif

void TextQuery::ReadFromFile(const string&filename){
    ifstream fr;
    fr.open(filename);
...
void TextQuery::getWordFromSet(const set<Paragragh>::iterator it){
        //cout<<it->_line<<":"<<it->_data<<endl;   
        //利用str存储每一段的内容
        ...
        if(strMap.end()==strMap.find(strBuffer2)){
            ...
            strMap.insert(pair<string,pair<int,int>>\
                (strBuffer2,make_pair(1,it->_line)));
        }else{
            /*第一行*/
            strMap.insert(pair<string,pair<int,int>>\
                (strBuffer2,make_pair(1,it->_line)));
